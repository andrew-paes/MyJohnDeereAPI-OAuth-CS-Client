...
namespace SampleApp.Sources.democlient
{
    public class FilesApiExamples
    {
        ...
        public void MakeFilesApiCalls(List<Organization> organizations)
        {
            ...
            var orgWithFileUploadLink = organizations.FirstOrDefault(org => org.links.Any(link => link.rel == LinkRel.uploadFile.ToString()));
            if (orgWithFileUploadLink != null)
            {
                UploadNewFileToOrganization(orgWithFileUploadLink);
            }
        }
